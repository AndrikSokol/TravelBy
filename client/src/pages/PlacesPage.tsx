...
const PlacesPage = () => {
  ...
  return (
    <div>
      <Dialog open={isOpen} onClose={() => setIsOpen(false)} className="flex  items-center absolute  top-0 bottom-0 left-0 right-0  bg-black/30">
        <Dialog.Panel className=" flex mx-auto  flex-col p-5 bg-white rounded-lg">
          <Dialog.Title className="text-black w-full text-center text-xl">Delete the place</Dialog.Title>
          <Dialog.Description>You sure to delete this place?</Dialog.Description>

          <button onClick={removePlace} className="bg-primary py-2 px-3 w-full rounded-xl my-5 text-white font-bold">
            Delete
          </button>
        </Dialog.Panel>
      </Dialog>
      ...
      <div className="w-full lg:w-[80%] mx-auto">
        {placesForUser !== undefined && (
          <div>
            <div className="text-center font-bold  text-lg lg:text-xl"> List Of All Added Places</div>
            ...
            <div>
              ...
              {placesForUser.length > 0 &&
                placesForUser.map((place: IPlaceData) => (
                  <div className="sm:flex  sm:gap-4 sm:justify-between shadow-lg border-t-2 border-t-rose-200  my-5 items-center rounded-lg" key={place._id}>
                    <div className="sm:flex gap-4 ">
                      {place.photos.length > 0 && (
                        <div className=" flex w-full sm:w-auto shrink-0 grow justify-center sm:justify-start">
                          <img className="object-cover rounded-l-lg  w-full h-96 sm:h-48 sm:w-48" src={`${BASEURL}/uploads/${place.photos[0]}`} alt="" />
                        </div>
                      )}
                    ...
                    <div className="p-3 flex  flex-col justify-center  items-start my-2 sm:my-0 gap-3 text-white">
                      <Link to={"/account/places/" + place._id} className="bg-primary w-full text-center py-2 px-6 rounded-md hover:opacity-95 ease-in-out">
                        Edit
                      </Link>
                      <button onClick={() => handleModal(place)} className="bg-primary w-full  text-center py-2 px-6 rounded-md hover:opacity-95 ease-in-out">
                        Delete
                      </button>
                    </div>
                  </div>
        ...
        <Link className="inline-flex gap-1 bg-primary text-white rounded-full py-2 pl-4 pr-6" to={"/account/places/new"}>
